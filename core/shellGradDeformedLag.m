function [grad,Hess]=shellGradDeformedLag( x_in, FVs, Topo, Ev, Ef, Eo, boundaryedges,options)
% Note: x is deformed Shell, while FVs hold all undeformed shells. 
% compute the cost and gradient of cost function wrt nodes
%% input
% xfull: init value for average, vertices plus 6 Lagrange input
...
nverts = size(FVs{1}.vertices, 1);
% recompute length, area, and dihedral angle
x = x_in(1:nverts*3);
xlag = x_in(nverts*3+1:end);
%xlag = options.LagFactor.*ones(6,1);
...
[ ~,grad,H ] = fastShellGradAndHessDef( x,FVs,Ev,Eo,Ef, mu,lambda,eta,datweights,boundaryedges );
%g = zeros(nverts, 3);
...
%     end
%     
%     if nargout > 0 % gradient required       
...
%     end
%     
%     if nargout > 1 % hessian required
% 
%         H1_fast = getEdgeHessVec(FVs{m}, FV_def,Ev,mu,datweights(m));      
%         H2_fast = getFaceHessVec(FVs{m},FV_def,lambda,datweights(m));                  
%         H3_fast = getAngleHessVec(FVs{m},FV_def,Ev,Eo,eta,boundaryedges,datweights(m));                   
%         H = H + (H1_fast + H2_fast + H3_fast);
%         %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     end
% end
...
%grad = g(:);
% add Lagrange
grad = addLagrangeConstraintCost(xlag, grad, FVs{1}, FV_def);
...
% add Lagrange
Hess = addLagrangeConstraintGrad(H, FVs{1}, FV_def);
%Hess = sparse(Hess);
