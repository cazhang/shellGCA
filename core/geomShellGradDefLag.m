function [grad,Hess]=geomShellGradDefLag( x_in, FVs, Topo, Ev, Ef, Eo, boundaryedges,options)
...
[ ~,grad,H ] = geomShellGradDef( x, FVs, Topo, Ev, Ef, Eo, boundaryedges,options );
if options.setBrd
    ...
    for i=1:length(bverts)
        vid = bverts(i);
        hess = eye(3);
        H = setToH(H, hess, vid, vid, nverts);
    end
end
